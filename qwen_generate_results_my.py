...
def get_plan_then_code(task_prompt, model="qwen-turbo"):  # or "qwen-plus", "qwen-turbo"
    ...
    plan_prompt = f"""Based on the following task description, please generate some test cases (<=3) that can test whether a program is valid.
...
{task_prompt}"""
    ...
    test_cases = plan_resp.output.choices[0].message.content.strip()

    # 2) Ask for code using that plan
    code_prompt = f"""Based on the following task description and test cases, complete the task and try test your code on the test cases.
...
{test_cases}"""
    ...
    # 2) Ask for code using that plan
    code_prompt = f"""Based on the following task description, test cases and original codes, complete the task.
...
with open("qwen_results_my.jsonl", "w") as f:
    for r in results:
